...
const MinimizableWebChat = (parameters) => {
  ...
  const webSpeechPonyfillFactory = useMemo(() => {
    if (options.speechCredentials && options.speechCredentials.region && options.speechCredentials.subscriptionKey) {
      return createCognitiveServicesSpeechServicesPonyfillFactory(options.credentials);
    }
    else {
