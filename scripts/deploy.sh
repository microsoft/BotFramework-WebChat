...
~/blobxfer upload --local-path $TRAVIS_BUILD_DIR/dist --remote-path $PACKAGE_NAME/$PACKAGE_VERSION --storage-account webchattest --storage-account-key $AZURE_BLOB_KEY

# If TRAVIS_TAG is present, it means this is going PRODUCTION
if [-n $TRAVIS_TAG]
then
# Upload to /latest/
~/blobxfer upload --local-path $TRAVIS_BUILD_DIR/dist --remote-path $PACKAGE_NAME/latest --storage-account webchattest --storage-account-key $AZURE_BLOB_KEY
fi
