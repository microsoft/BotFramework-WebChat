...
var Attachments = function (props) {
    ...
    return attachmentLayout === 'carousel' ?
        React.createElement(Carousel_1.Carousel, tslib_1.__assign({ attachments: attachments, disabled: props.disabled }, otherProps))
        :
            React.createElement("div", { className: "wc-list" }, attachments.map(function (attachment, index) {
                return React.createElement(Attachment_1.AttachmentView, { attachment: attachment, format: props.format, disabled: props.disabled, key: index, onCardAction: props.onCardAction, onImageLoad: props.onImageLoad });
            }));
};
        ...
        switch (activity.type) {
            case 'message':
                return (React.createElement("div", null,
                    ...
                    React.createElement(Attachments, { attachments: activity.attachments, attachmentLayout: activity.attachmentLayout, disabled: props.disabled, format: props.format, onCardAction: props.onCardAction, onImageLoad: props.onImageLoad, size: props.size })));
            case 'typing':
