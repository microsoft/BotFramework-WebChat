...
var Video = function (props) {
    ...
    switch (url.hostname) {
        ...
        case YOUTUBE_WWW_SHORT_DOMAIN:
            return (React.createElement(Youtube, { autoPlay: !props.disabled && props.autoPlay, embedId: url.hostname === YOUTUBE_DOMAIN || url.hostname === YOUTUBE_WWW_DOMAIN ? urlQueryParams.v : pathSegments[pathSegments.length - 1], loop: props.loop }));
        case VIMEO_WWW_DOMAIN:
        case VIMEO_DOMAIN:
            return (React.createElement(Vimeo, { autoPlay: !props.disabled && props.autoPlay, embedId: pathSegments[pathSegments.length - 1], loop: props.loop }));
        default:
...
var Media = function (props) {
    switch (props.type) {
        ...
        case 'audio':
            return (React.createElement("audio", tslib_1.__assign({ controls: true }, props, { autoPlay: !props.disabled && props.autoPlay })));
        default:
...
exports.AttachmentView = function (props) {
    ...
    var onCardAction = function (cardAction) { return cardAction && !props.disabled &&
        (function (e) {
            props.onCardAction(cardAction.type, cardAction.value);
            e.stopPropagation();
        }); };
    var attachedImage = function (images) { return images && images.length > 0 &&
        React.createElement(Media, { alt: images[0].alt, disabled: props.disabled, onClick: onCardAction(images[0].tap), onLoad: props.onImageLoad, src: images[0].url }); };
    var getRichCardContentMedia = function (type, content) {
        ...
        return content.media.map(function (md, i) {
            ...
            return (React.createElement(Media, { autoPlay: content.autostart, disabled: props.disabled, key: i, loop: content.autoloop, onLoad: props.onImageLoad, poster: content.image && content.image.url, src: md.url, type: t }));
        });
    };
    switch (attachment.contentType) {
        case 'application/vnd.microsoft.card.hero':
            ...
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "hero", disabled: props.disabled, nativeCard: heroCardBuilder_1.card, onCardAction: props.onCardAction, onClick: onCardAction(attachment.content.tap), onImageLoad: props.onImageLoad }));
        case 'application/vnd.microsoft.card.thumbnail':
            ...
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "thumbnail", disabled: props.disabled, nativeCard: thumbnailCardBuilder.card, onCardAction: props.onCardAction, onClick: onCardAction(attachment.content.tap), onImageLoad: props.onImageLoad }));
        case 'application/vnd.microsoft.card.video':
            if (!attachment.content || !attachment.content.media || attachment.content.media.length === 0) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "video", disabled: props.disabled, nativeCard: CardBuilder.buildCommonCard(attachment.content), onCardAction: props.onCardAction }, getRichCardContentMedia('video', attachment.content)));
        case 'application/vnd.microsoft.card.animation':
            if (!attachment.content || !attachment.content.media || attachment.content.media.length === 0) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "animation", disabled: props.disabled, onCardAction: props.onCardAction, nativeCard: CardBuilder.buildCommonCard(attachment.content) }, getRichCardContentMedia(mediaType, attachment.content)));
        case 'application/vnd.microsoft.card.audio':
            if (!attachment.content || !attachment.content.media || attachment.content.media.length === 0) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "audio", disabled: props.disabled, nativeCard: CardBuilder.buildCommonCard(attachment.content), onCardAction: props.onCardAction }, getRichCardContentMedia('audio', attachment.content)));
        case 'application/vnd.microsoft.card.signin':
            if (!attachment.content) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "signin", disabled: props.disabled, nativeCard: CardBuilder.buildCommonCard(attachment.content), onCardAction: props.onCardAction }));
        case 'application/vnd.microsoft.card.oauth':
            if (!attachment.content) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "signin", disabled: props.disabled, nativeCard: CardBuilder.buildOAuthCard(attachment.content), onCardAction: props.onCardAction }));
        case 'application/vnd.microsoft.card.receipt':
            ...
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "receipt", disabled: props.disabled, nativeCard: receiptCardBuilder_1.card, onCardAction: props.onCardAction, onClick: onCardAction(attachment.content.tap) }));
        case 'application/vnd.microsoft.card.adaptive':
            if (!attachment.content) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { disabled: props.disabled, jsonCard: attachment.content, onCardAction: props.onCardAction, onImageLoad: props.onImageLoad }));
        // Deprecated format for Skype channels. For testing legacy bots in Emulator only.
        case 'application/vnd.microsoft.card.flex':
            if (!attachment.content) {
                return null;
            }
            return (React.createElement(AdaptiveCardContainer_1.default, { className: "flex", disabled: props.disabled, nativeCard: CardBuilder.buildCommonCard(attachment.content), onCardAction: props.onCardAction }, attachedImage(attachment.content.images)));
        case 'image/svg+xml':
        case 'image/png':
        case 'image/jpg':
        case 'image/jpeg':
        case 'image/gif':
            return (React.createElement(Media, { disabled: props.disabled, onLoad: props.onImageLoad, src: attachment.contentUrl }));
        case 'audio/mpeg':
        case 'audio/mp4':
            return (React.createElement(Media, { disabled: props.disabled, src: attachment.contentUrl, type: "audio" }));
        case 'video/mp4':
            return (React.createElement(Media, { disabled: props.disabled, onLoad: props.onImageLoad, poster: attachment.thumbnailUrl, src: attachment.contentUrl, type: "video" }));
        default:
            var unknownAttachment = props.attachment;
            return (React.createElement(Unknown, { contentType: unknownAttachment.contentType, contentUrl: unknownAttachment.contentUrl, format: props.format, name: unknownAttachment.name }));
    }
};
