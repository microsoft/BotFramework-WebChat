import { Graph, slantNode } from '@msinternal/botframework-webchat-core-graph';
...
function createGraphFromStore(store: ReturnType<typeof createStore>): Graph {
  ...
  const channelAudience = parse(slantNode(), {
  ...
  const othersAudience = parse(slantNode(), {
    '@context': 'https://schema.org',
  ...
  const selfAudience = parse(slantNode(), {
    '@context': 'https://schema.org',
    '@id': '_:audience/user',
    '@type': ['Audience'],
    audienceType: ['self']
  ...
  graph.upsert(othersAudience, channelAudience, selfAudience);
  ...
  store.subscribe(() => {
    ...
    for (const activity of addedActivities) {
      graph.upsert({
        ...
        encodingFormat: 'textFormat' in activity && activity.textFormat !== 'markdown' ? 'text/plain' : 'text/markdown',
