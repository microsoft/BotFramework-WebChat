...
import { useCodeHighlighterWithTransform } from '../../../hooks/internal/codeHighlighter/useCodeHighlighterWithTransform';
...
const ViewCodeButton = ({ className, code, language, title = '', isAIGenerated = false }: Props) => {
  ...
  const highlightCode = useCodeHighlighterWithTransform();

  const showCodeModal = useCallback(() => {
    showModal(
      () =>
        code && (
          <CodeContent highlightedCode={highlightCode(code, language, { theme: codeBlockTheme })} title={title}>
            {isAIGenerated && (
  ...
  }, [showModal, viewCodeDialog, localize, title, code, highlightCode, language, codeBlockTheme, isAIGenerated]);
