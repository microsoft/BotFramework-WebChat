...
test('useMemoize should cache result if deps change', () => {
  const expensiveSum = jest.fn((x, y) => x + y);
...
test('useMemoize should rely on fn to fetch cache', () => {
  const expensiveSum1 = jest.fn((x, y) => x + y);
  const expensiveSum2 = jest.fn((x, y) => x + y);
  ...
  expect(expensiveSum1).toHaveBeenCalledTimes(2);

  expect(expensiveSum1.mock.calls[0]).toEqual([1, 2]);
  expect(expensiveSum1.mock.calls[1]).toEqual([2, 4]);

  // After the run, 1 + 2 = 3, and 2 + 4 = 6 is cached.

  // Start another run with the 2nd fn
  useMemoize(
    expensiveSum2,
    sum => {
      expect(sum(1, 2)).toBe(3); // Not cached, return 3.
  ...
  expect(expensiveSum1).toHaveBeenCalledTimes(2);
  expect(expensiveSum2).toHaveBeenCalledTimes(2);
  expect(expensiveSum2.mock.calls[1]).toEqual([3, 6]);

  // Start another run with previous cache for the 1st fn
  useMemoize(
    expensiveSum1,
    sum => {
